...
const LinkVerificationSystem = require('./linkVerification');
require('dotenv').config();

// Initialize link verification system
const linkVerifier = new LinkVerificationSystem()
...
const SECURITY_VIOLATION_MESSAGE = "ULTIMATE ANTI-NUKE ACTIVATED - You have been permanently banned for attempting unauthorized server modifications. This server is protected by script.agi with zero tolerance policy.";
...
function isAuthorized(message) {
    ...
    const isInOwnerChannel = message.channel.id === '1380640369506521258';
    ...
    // Server owner can only use commands in designated channels
    return isServerOwner && (isInOwnerChannel || isInAdminChannel);
}
...
function getTotalCommandsCount() {
    ...
    const commands = [
        ...
        'addrole', 'removerole', 'dm', 'ui', 'ping', 'help', 'dev', 'roles',
        // Interim Role Management
        'prmtr', 'revtr', 'remtr', 'addtr', 'sendinterim', 'intrch', 'intrm',
        // Bot & User Protection
        'whitelist', 'flagged', 'unflag', 'scanserver', 'purgebots', 'unfu', 'flaggedusers',
        // Server Protection & Templates
        'protection', 'createbaseline', 'srvcrt', 'mdfsrv', 'mdfsv', 'clstr', 'optr',
        // Emergency & Configuration Commands
        'panic', 'stop panic', 'emergency', 'end emergency', 'wbtestan', 'set',
        'setinterimrole', 'recovery', 'nightmode',
...
function isProtectionImmune(userId, guildId) {
    // ONLY SERVER OWNER AND BOT ARE IMMUNE - NO EXCEPTIONS
    ...
    return isBotOwner || isServerOwner || isBot;
}
...
// Function to handle protection violation - ULTIMATE ANTI-NUKE SECURITY MODE
async function handleProtectionViolation(guild, user, violationType, details) {
    // Skip if user is immune to protection - ONLY OWNER AND BOT
    if (isProtectionImmune(user.id, guild.id)) {
        console.log(`🛡️ User ${user.username} is immune to protection - allowing action`);
        return;
    }

    console.log(`🚨 ULTIMATE ANTI-NUKE VIOLATION: ${violationType} by ${user.username} - IMMEDIATE BAN`);

    // IMMEDIATE BAN FOR ANY UNAUTHORIZED CHANGE
    try {
        const member = await guild.members.fetch(user.id).catch(() => null);
        if (member) {
            // IMMEDIATE PERMANENT BAN - NO MERCY
            try {
                await guild.bans.create(user.id, {
                    reason: `ULTIMATE ANTI-NUKE: ${violationType} - ${details}`,
                ...
                try {
                    const banDmEmbed = new EmbedBuilder()
                        .setColor('#000000')
                        .setTitle('⚫ ULTIMATE ANTI-NUKE SECURITY - PERMANENT BAN')
                        .setDescription('**YOU HAVE BEEN PERMANENTLY BANNED FOR ATTEMPTING TO BYPASS SECURITY**\n\nFuck You Motherfucker, Don\'t even think of nuke you will fucked by script.agi')
                        .addFields(
                            { name: '⚠️ Violation', value: `${violationType}`, inline: true },
                            { name: '⏰ Ban Duration', value: 'PERMANENT', inline: true },
                            { name: '🛡️ Security Level', value: 'ULTIMATE ANTI-NUKE', inline: true },
                            { name: '📋 Server Policy', value: 'ZERO TOLERANCE - NO UNAUTHORIZED CHANGES ALLOWED', inline: false },
                            { name: '🚨 Final Warning', value: 'This server is protected by script.agi - impossible to bypass', inline: false }
                        )
                        ...
                        .setFooter({ text: 'Script.AGI Ultimate Anti-Nuke System - Zero Tolerance' })
                        .setTimestamp();

                    await user.send({ embeds: [banDmEmbed] });
                    console.log(`✅ Sent ban notification DM to ${user.username}`);
                } catch (dmError) {
                    console.log(`Could not send ban DM to ${user.username}:`, dmError.message);
                }
            } catch (banError) {
                console.error('Error banning user:', banError);
                // If ban fails, try quarantine as backup
                const quarantineSuccess = await quarantineUser(
                    member,
                    `ULTIMATE ANTI-NUKE BACKUP: ${violationType} - ${details}`,
                    MAXIMUM_SECURITY_QUARANTINE_DURATION // 2 hours
                );
                if (quarantineSuccess) {
                    console.log(`✅ User ${user.username} quarantined as backup for: ${violationType}`);
                }
            }
        } else {
            // User not in server but still log the attempt
            console.log(`⚠️ User ${user.username} not in server but violation logged`);
        }
    } catch (error) {
        console.error('Error in ultimate anti-nuke violation handler:', error);
    }

    // Create comprehensive anti-nuke security alert for logs
    const ultimateSecurityEmbed = new EmbedBuilder()
        .setColor('#000000')
        .setTitle('⚫ ULTIMATE ANTI-NUKE SECURITY - IMMEDIATE BAN EXECUTED')
        .setDescription(`**ZERO TOLERANCE SECURITY BREACH DETECTED AND ELIMINATED**`)
        .addFields(
            { name: '🎯 Violator', value: `${user.username}\n\`${user.id}\``, inline: true },
            { name: '⚠️ Violation Type', value: violationType, inline: true },
            { name: '⏰ Punishment', value: 'PERMANENT BAN', inline: true },
            { name: '📝 Details', value: details, inline: false },
            { name: '🛡️ Actions Taken', value: '✅ User permanently banned\n✅ 7 days messages deleted\n✅ Ban notification sent\n✅ Server protected', inline: false },
            { name: '🔴 Security Policy', value: '**ZERO TOLERANCE - ULTIMATE ANTI-NUKE ACTIVE**', inline: false },
            { name: '🚨 Security Status', value: '⚫ **ULTIMATE PROTECTION - IMPOSSIBLE TO BYPASS**', inline: true }
        )
        .setThumbnail(user.displayAvatarURL({ dynamic: true, size: 128 }))
        .setFooter({ text: 'Script.AGI Ultimate Anti-Nuke System - Zero Tolerance Policy' })
        .setTimestamp();

    await sendLogMessage(guild, ultimateSecurityEmbed);
}
...
async function handleUnauthorizedBot(guild, bot, inviter) {
    try {
        // Check if inviter is authorized (server owner or bot owner can add any bots)
        if (inviter && (inviter.id === guild.ownerId || inviter.id === BOT_OWNER_ID)) {
            console.log(`✅ AUTHORIZED USER ADDED BOT: ${inviter.username} added ${bot.user.username} - ALLOWING`);
            
            // Log the authorized addition
            const authorizedBotEmbed = new EmbedBuilder()
                .setColor('#00FF00')
                .setTitle('✅ AUTHORIZED BOT ADDITION')
                .setDescription(`**Authorized user added a bot** - No action taken`)
                .addFields(
                    { name: '🤖 Bot Information', value: `**Name:** ${bot.user.username}\n**ID:** \`${bot.user.id}\``, inline: true },
                    { name: '👤 Added By', value: `**User:** ${inviter.username}\n**ID:** \`${inviter.id}\`\n**Status:** Authorized`, inline: true },
                    { name: '✅ Authorization', value: inviter.id === guild.ownerId ? '👑 Server Owner' : '🤖 Bot Owner', inline: true },
                    { name: '📋 Whitelist Status', value: WHITELISTED_BOTS.has(bot.user.id) ? '✅ Already whitelisted' : '⚠️ Not whitelisted but allowed', inline: false },
                    { name: '🛡️ Action Taken', value: '✅ No action - Authorized addition\n✅ Bot allowed to stay', inline: false }
                )
                .setThumbnail(bot.user.displayAvatarURL({ dynamic: true, size: 128 }))
                .setFooter({ text: 'Server Protection System - Authorized Addition' })
                .setTimestamp();

            await sendLogMessage(guild, authorizedBotEmbed);
            return false; // No unauthorized action detected
        }

        console.log(`🚨 UNAUTHORIZED BOT DETECTED: ${bot.user.username} (${bot.user.id}) added by unauthorized user`);
        ...
        const unauthorizedBotEmbed = new EmbedBuilder()
            .setColor('#FF0000')
            .setTitle('🚨 UNAUTHORIZED BOT BLOCKED - BOTH BOT AND INVITER BANNED')
            .setDescription(`**SECURITY ALERT:** Unauthorized bot integration attempt detected and blocked! Both bot and inviter have been permanently banned.`)
            .addFields(
                { name: '🤖 Bot Information', value: `**Name:** ${bot.user.username}\n**ID:** \`${bot.user.id}\`\n**Status:** Kicked and banned permanently`, inline: true },
                { name: '👤 Inviter Information', value: inviter ? `**User:** ${inviter.username}\n**ID:** \`${inviter.id}\`\n**Status:** ${inviter.id !== guild.ownerId && inviter.id !== BOT_OWNER_ID ? 'Banned permanently' : 'Owner - Cannot ban'}` : 'Unknown inviter', inline: true },
                { name: '🛡️ Protection Actions Taken', value: '✅ Bot kicked immediately\n✅ Bot banned permanently\n✅ Bot flagged globally\n✅ Inviter banned permanently\n✅ Integration terminated\n✅ Both threats neutralized', inline: false },
                { name: '📋 Whitelist Status', value: `❌ Bot ID not in approved whitelist\n**Total Whitelisted:** ${WHITELISTED_BOTS.size} bots`, inline: false },
                { name: '⚠️ Security Level', value: '🔴 **MAXIMUM THREAT - DUAL BAN**', inline: true },
                { name: '🎯 Status', value: '✅ **BOTH THREATS ELIMINATED**', inline: true }
            )
            .setThumbnail(bot.user.displayAvatarURL({ dynamic: true, size: 128 }))
            .setFooter({ text: 'Server Protection System - Dual Ban Protocol Active' })
            .setTimestamp();
        ...
        try {
            await guild.bans.create(bot.user.id, {
                reason: `UNAUTHORIZED BOT - Not in whitelist. Added by unauthorized user. Auto-banned by protection system.`,
                deleteMessageSeconds: 7 * 24 * 60 * 60 // Delete 7 days of messages
            });
        ...
        // BAN the inviter permanently (both bot and inviter get banned as requested)
        if (inviter) {
            // Only skip banning if the inviter is the server owner or bot owner
            if (inviter.id !== guild.ownerId && inviter.id !== BOT_OWNER_ID) {
                try {
                    // Permanent ban for the inviter who added unauthorized bot
                    await guild.bans.create(inviter.id, {
                        reason: `SECURITY VIOLATION: Added unauthorized bot ${bot.user.username} (${bot.user.id}). Both bot and inviter banned permanently.`,
                        deleteMessageSeconds: 7 * 24 * 60 * 60 // Delete 7 days of messages
                    });
                    console.log(`✅ Permanently banned bot inviter: ${inviter.username}`);

                    // Send aggressive warning DM to inviter with custom message and GIF
                    try {
                        const warningEmbed = new EmbedBuilder()
                            .setColor('#FF0000')
                            .setTitle('🚨 FUCK YOU MOTHERFUCKER - PERMANENTLY BANNED')
                            ...
                            .addFields(
                                ...
                                { name: '⚠️ Final Warning', value: 'You have been permanently banned for adding unauthorized bot. Both you and the bot are banned.', inline: false }
                            )
                            .setImage('https://cdn.discordapp.com/attachments/1377710452653424711/1411748251920765018/have-a-nice-day-fuck-you.gif?ex=68b5c884&is=68b47704&hm=d98f09f00526750721143f5b4757363b262542a34426d9e10dac5d25a1d39741&')
                            .setFooter({ text: 'Script.AGI Maximum Security System - Permanent Ban' })
                            .setTimestamp();

                        await inviter.send({ embeds: [warningEmbed] });
                        console.log(`🚨 Sent permanent ban DM with GIF to ${inviter.username}`);
                    } catch (dmError) {
                        console.log('Could not send ban DM to bot inviter:', dmError.message);
                    }
                } catch (inviterError) {
                    console.error('Error banning bot inviter:', inviterError);
                    
                    // Fallback to quarantine if ban fails
                    try {
                        const inviterMember = await guild.members.fetch(inviter.id).catch(() => null);
                        if (inviterMember) {
                            const quarantineSuccess = await quarantineUser(
                                inviterMember,
                                `CRITICAL: Attempted to add unauthorized bot: ${bot.user.username} (${bot.user.id}) - Ban failed, quarantined instead`,
                                1440 // 24 hours quarantine as fallback
                            );

                            if (quarantineSuccess) {
                                console.log(`✅ Quarantined bot inviter as fallback: ${inviter.username}`);
                            }
                        }
                    } catch (fallbackError) {
                        console.error('Error with fallback quarantine:', fallbackError);
                    }
                }
            } else {
                console.log(`⚠️ Inviter ${inviter.username} is server/bot owner - cannot ban, but unauthorized bot was still removed`);
            }
        } else {
            console.log(`⚠️ Could not identify who added the unauthorized bot ${bot.user.username} - bot was removed but inviter unknown`);
        }
...
// Checking request system configuration
const CHECKING_REQUEST_CHANNEL_ID = '1414936491356065845';
const CHECKER_ROLE_ID = '1414331740109017209';
const CHECKING_LOGS_CHANNEL_ID = '1384079807335239690';
const CHECKING_APPROVAL_CHANNEL_ID = '1414946055350521897';
const CHECKING_VOICE_CHANNEL_ID = '1386448616217907271';

// Store checking requests
const checkingRequests = new Map(); // requestId -> { userId, device, timestamp, status, checkers, approvalData }
const checkingWidgetMessageIds = new Map(); // channelId -> messageId
...
// Function to create Help Card 2 - Interim Role Management
function createHelpCard2() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setTitle('ᯓᡣ𐭩 **Interim Role Management System**')
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
        ...
        .setImage('https://cdn.discordapp.com/attachments/1377710452653424711/1410001205639254046/a964ff33-1eaf-49ed-b487-331b3ffe3ebd.gif')
        .setFooter({
            text: 'Card 2/10 • Interim Role Management',
            iconURL: 'https://cdn.discordapp.com/attachments/1377710452653424711/1410001205639254046/a964ff33-1eaf-49ed-b487-331b3ffe3ebd.gif'
        })
        .setTimestamp();
...
// Function to create Help Card 3 - Voice Management System
function createHelpCard3() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setTitle('ᯓᡣ𐭩 **Voice Management System**')
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `ᡣ𐭩 \`hvcm\` - Voice management help`
        )
        .setThumbnail(client.user.displayAvatarURL({ dynamic: true, size: 256 }))
        .setImage('https://cdn.discordapp.com/attachments/1377710452653424711/1410001205639254046/a964ff33-1eaf-49ed-b487-331b3ffe3ebd.gif')
        .setFooter({
            text: 'Card 3/10 • Voice Management System',
            iconURL: 'https://cdn.discordapp.com/attachments/1377710452653424711/1410001205639254046/a964ff33-1eaf-49ed-b487-331b3ffe3ebd.gif'
        })
        .setTimestamp();
...
// Function to show help with slideshow functionality - no buttons
async function showHelp(guildId = null, cardNumber = 1, userId = null, interactionId = null) {
    ...
    // Return help embed without any buttons/components
    return { embeds: [helpEmbed], components: [] };
}
...
// Function to send/update checking request widget
async function sendCheckingRequestWidget(guild) {
    try {
        const checkingChannel = guild.channels.cache.get(CHECKING_REQUEST_CHANNEL_ID);
        if (!checkingChannel) {
            console.error('Checking request channel not found');
            return false;
        }

        const embedDescription = `🔍 **CHECKING REQUEST SYSTEM** 🔍

**Professional Checking Service**

Need someone checked? Our expert checkers are here to help! Click the button below to submit a checking request.

**How it works:**
1. Click "Request Checking" button
2. Select your device (Mobile/PC)
3. Checkers get notified instantly
4. Approval process begins
5. Connect to voice channel when approved
...
Click the button below to start your checking request!`;

        const checkingEmbed = new EmbedBuilder()
            .setColor('#00FF00')
            .setAuthor({
                name: 'Professional Checking Service - Discord.gg/scriptspace',
                iconURL: guild.iconURL({ dynamic: true })
            })
            .setTitle('🔍 **Checking Request System**')
            .setDescription(embedDescription)
            .setThumbnail(client.user.displayAvatarURL({ dynamic: true, size: 256 }))
            .setFooter({
                text: 'Checking Request System • Professional Service',
                iconURL: client.user.displayAvatarURL({ size: 16 })
            })
            .setTimestamp();

        const checkingButton = new ButtonBuilder()
            .setCustomId('request_checking')
            .setLabel('🔍 Request Checking')
            .setStyle(ButtonStyle.Primary);

        const buttonRow = new ActionRowBuilder()
            .addComponents(checkingButton);

        // Look for existing checking widget message
        let existingMessage = null;
        const storedMessageId = checkingWidgetMessageIds.get(CHECKING_REQUEST_CHANNEL_ID);

        if (storedMessageId) {
            try {
                existingMessage = await checkingChannel.messages.fetch(storedMessageId);
            } catch (error) {
                checkingWidgetMessageIds.delete(CHECKING_REQUEST_CHANNEL_ID);
            }
        }

        if (!existingMessage) {
            const messages = await checkingChannel.messages.fetch({ limit: 50 });
            existingMessage = messages.find(msg =>
                msg.author.id === client.user.id &&
                msg.embeds.length > 0 &&
                msg.embeds[0].title &&
                msg.embeds[0].title.includes('Checking Request System')
            );

            if (existingMessage) {
                checkingWidgetMessageIds.set(CHECKING_REQUEST_CHANNEL_ID, existingMessage.id);
            }
        }
        ...
        if (existingMessage) {
            try {
                ...
                console.log('Checking request widget updated successfully');
                return true;
            } catch (error) {
                console.error('Error updating checking widget:', error);
        ...
        if (!existingMessage) {
            try {
                const newMessage = await checkingChannel.send(messageData);
                checkingWidgetMessageIds.set(CHECKING_REQUEST_CHANNEL_ID, newMessage.id);
                console.log('New checking request widget sent successfully');
                return true;
            } catch (error) {
                console.error('Error sending checking widget:', error);
                return false;
            }
        }

    } catch (error) {
        console.error('Error in sendCheckingRequestWidget:', error);
        return false;
    }
}

// Function to create checking approval widget
async function createCheckingApprovalWidget(requestId, guild, requester, device) {
    try {
        const approvalChannel = guild.channels.cache.get(CHECKING_APPROVAL_CHANNEL_ID);
        if (!approvalChannel) {
            console.error('Checking approval channel not found');
            return false;
        }

        const approvalEmbed = new EmbedBuilder()
            .setColor('#FFA500')
            .setTitle('⏳ Checking Request - Awaiting Approval')
            .setDescription(`**A user has requested checking service**\n\nPlease review and approve the checking request below.`)
            .addFields(
                { name: '👤 User', value: `${requester.username}\n\`${requester.id}\``, inline: true },
                { name: '📱 Device', value: device.toUpperCase(), inline: true },
                { name: '⏰ Requested At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: true },
                { name: '🆔 Request ID', value: `\`${requestId}\``, inline: true },
                { name: '📊 Status', value: 'Pending checker approval', inline: true },
                { name: '🔍 Action Required', value: 'Checkers can approve or set delay time', inline: false }
            )
            .setThumbnail(requester.displayAvatarURL({ dynamic: true, size: 128 }))
            .setFooter({ text: `Checking Request System • ID: ${requestId}` })
            .setTimestamp();

        const approveButton = new ButtonBuilder()
            .setCustomId(`approve_checking_${requestId}`)
            .setLabel('✅ Approve Checking')
            .setStyle(ButtonStyle.Success);

        const disapproveButton = new ButtonBuilder()
            .setCustomId(`disapprove_checking_${requestId}`)
            .setLabel('❌ Disapprove Checking')
            .setStyle(ButtonStyle.Danger);

        const delay5Button = new ButtonBuilder()
            .setCustomId(`delay_checking_${requestId}_5`)
            .setLabel('⏰ 5 mins delay')
            .setStyle(ButtonStyle.Secondary);

        const delay10Button = new ButtonBuilder()
            .setCustomId(`delay_checking_${requestId}_10`)
            .setLabel('⏰ 10 mins delay')
            .setStyle(ButtonStyle.Secondary);

        const delay20Button = new ButtonBuilder()
            .setCustomId(`delay_checking_${requestId}_20`)
            .setLabel('⏰ 20 mins delay')
            .setStyle(ButtonStyle.Secondary);

        const delay30Button = new ButtonBuilder()
            .setCustomId(`delay_checking_${requestId}_30`)
            .setLabel('⏰ 30 mins delay')
            .setStyle(ButtonStyle.Secondary);

        const busyButton = new ButtonBuilder()
            .setCustomId(`busy_checking_${requestId}`)
            .setLabel('😴 I am busy')
            .setStyle(ButtonStyle.Secondary);
        ...
        const row2 = new ActionRowBuilder()
            .addComponents(delay5Button, delay10Button, delay20Button);

        const row3 = new ActionRowBuilder()
            .addComponents(delay30Button, busyButton);
        ...
        console.error('Error creating checking approval widget:', error);
        return null;
    }
}

// Function to notify checkers via DM
async function notifyCheckers(guild, requester, device, requestId) {
    try {
        const checkerRole = guild.roles.cache.get(CHECKER_ROLE_ID);
        if (!checkerRole) {
            console.error('Checker role not found');
            return false;
        }

        const checkers = checkerRole.members;
        let notifiedCount = 0;

        const notificationEmbed = new EmbedBuilder()
            .setColor('#00FF00')
            .setTitle('🔍 New Checking Request')
            .setDescription(`A user has requested checking service and needs your attention!`)
            .addFields(
                { name: '👤 User', value: `${requester.username}`, inline: true },
                { name: '📱 Device', value: device.toUpperCase(), inline: true },
                { name: '⏰ Time', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                { name: '📍 Approval Channel', value: `<#${CHECKING_APPROVAL_CHANNEL_ID}>`, inline: true },
                { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: true },
                { name: '🆔 Request ID', value: `\`${requestId}\``, inline: true },
                { name: '⚡ Action', value: 'Go to approval channel to respond', inline: false }
            )
            .setThumbnail(requester.displayAvatarURL({ dynamic: true, size: 128 }))
            .setFooter({ text: 'Checking Request System - New Request' })
            .setTimestamp();

        for (const checker of checkers.values()) {
            try {
                if (!checker.user.bot) {
                    await checker.user.send({ embeds: [notificationEmbed] });
                    notifiedCount++;
                }
            } catch (error) {
                console.log(`Could not send DM to checker ${checker.user.username}:`, error.message);
            }
        }

        console.log(`✅ Notified ${notifiedCount} checkers about request ${requestId}`);
        return notifiedCount > 0;

    } catch (error) {
        console.error('Error notifying checkers:', error);
        return false;
    }
}

// Function to handle checking request approval
async function handleCheckingApproval(requestId, guild, approver, approvalType, delayMinutes = 0) {
    try {
        const requestData = checkingRequests.get(requestId);
        if (!requestData) {
            console.error(`Checking request ${requestId} not found`);
            return false;
        }

        const requester = await client.users.fetch(requestData.userId);
        const logsChannel = guild.channels.cache.get(CHECKING_LOGS_CHANNEL_ID);

        if (approvalType === 'approve') {
            // Update request status
            requestData.status = 'approved';
            requestData.approvedBy = approver.id;
            requestData.approvedAt = Date.now();
            checkingRequests.set(requestId, requestData);

            // Send approval DM to requester
            const approvalDM = new EmbedBuilder()
                .setColor('#00FF00')
                .setTitle('✅ Checking Request Approved!')
                .setDescription(`Great news! Your checking request has been accepted by our checkers.`)
                .addFields(
                    { name: '✅ Approved By', value: `${approver.username}`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: false },
                    { name: '🔗 Join Voice', value: `Connect to the voice channel now for your checking!`, inline: false }
                )
                .setThumbnail(guild.iconURL({ dynamic: true, size: 128 }))
                .setFooter({ text: 'Checking Service - Approved' })
                .setTimestamp();

            try {
                await requester.send({ embeds: [approvalDM] });
            } catch (dmError) {
                console.log('Could not send approval DM:', dmError.message);
            }

            // Log to checking logs channel
            if (logsChannel) {
                const logEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('✅ Checking Request Approved')
                    .setDescription(`Checking request has been approved and user notified.`)
                    .addFields(
                        { name: '👤 User', value: `${requester.username} (\`${requester.id}\`)`, inline: true },
                        { name: '✅ Approved By', value: `${approver.username}`, inline: true },
                        { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                        { name: '⏰ Approved At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                        { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: true },
                        { name: '📊 Status', value: 'User can now join voice', inline: true }
                    )
        ...
        } else if (approvalType === 'disapprove') {
            // Update request status to fully disapproved (only used when manually closing request)
            requestData.status = 'disapproved';
            requestData.disapprovedBy = [approver.id];
            requestData.disapprovedAt = Date.now();
            checkingRequests.set(requestId, requestData);

            // Send disapproval DM to requester
            const disapprovalDM = new EmbedBuilder()
                .setColor('#FF0000')
                .setTitle('❌ Checking Request Disapproved')
                .setDescription(`Your checking request has been reviewed and disapproved.`)
                .addFields(
                    { name: '❌ Reviewed By', value: `${approver.username}`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '📊 Status', value: 'Request denied', inline: true },
                    { name: '💡 Suggestion', value: 'You can submit a new request later', inline: false }
                )
                .setFooter({ text: 'Checking Service - Disapproved' })
                .setTimestamp();

            try {
                await requester.send({ embeds: [disapprovalDM] });
            } catch (dmError) {
                console.log('Could not send disapproval DM:', dmError.message);
            }

        } else if (approvalType === 'disapprove_individual') {
            // Individual checker disapproval - don't close the request
            // Send notification DM to requester
            const individualDisapprovalDM = new EmbedBuilder()
                .setColor('#FFA500')
                .setTitle('⏰ Checking Request Update')
                .setDescription(`One checker has declined your request, but it remains open for other checkers.`)
                .addFields(
                    { name: '❌ Declined By', value: `${approver.username}`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '📊 Status', value: 'Still pending - waiting for other checkers', inline: true },
                    { name: '💡 Note', value: 'Other checkers can still approve your request', inline: false }
                )
                .setFooter({ text: 'Checking Service - Individual Response' })
                .setTimestamp();

            try {
                await requester.send({ embeds: [individualDisapprovalDM] });
            } catch (dmError) {
                console.log('Could not send individual disapproval DM:', dmError.message);
            }

        } else if (approvalType === 'delay') {
            // Send delay notification DM to requester
            const delayDM = new EmbedBuilder()
                .setColor('#FFA500')
                .setTitle('⏰ Checking Request - Delay Notice')
                .setDescription(`Your checking request has been acknowledged. Please wait for the specified time.`)
                .addFields(
                    { name: '👤 Checker', value: `${approver.username}`, inline: true },
                    { name: '⏰ Delay Time', value: `${delayMinutes} minutes`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '📊 Status', value: `Please wait ${delayMinutes} minutes`, inline: false },
                    { name: '💡 Note', value: 'Your request is still active, just delayed', inline: false }
                )
                .setFooter({ text: 'Checking Service - Delay Notice' })
                .setTimestamp();

            try {
                await requester.send({ embeds: [delayDM] });
            } catch (dmError) {
                console.log('Could not send delay DM:', dmError.message);
            }

        } else if (approvalType === 'busy') {
            // Send busy notification DM to requester
            const busyDM = new EmbedBuilder()
                .setColor('#9B59B6')
                .setTitle('😴 Checker Currently Busy')
                .setDescription(`One of our checkers is currently busy but has acknowledged your request.`)
                .addFields(
                    { name: '👤 Checker', value: `${approver.username}`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '📊 Status', value: 'Waiting for available checker', inline: true },
                    { name: '💡 Note', value: 'Your request remains active in the queue', inline: false }
                )
                .setFooter({ text: 'Checking Service - Checker Busy' })
                .setTimestamp();

            try {
                await requester.send({ embeds: [busyDM] });
            } catch (dmError) {
                console.log('Could not send busy DM:', dmError.message);
            }
        }

        // Always log the action
        if (logsChannel) {
            let logColor = '#FFA500'; // Default orange
            let logTitle = '';
            let statusText = '';
            ...
            if (approvalType === 'approve') {
                logColor = '#00FF00';
                logTitle = 'APPROVED';
                statusText = 'Approved and user notified';
            } else if (approvalType === 'disapprove') {
                logColor = '#FF0000';
                logTitle = 'DISAPPROVED';
                statusText = 'Request closed and user notified';
            } else if (approvalType === 'disapprove_individual') {
                logColor = '#FFA500';
                logTitle = 'INDIVIDUAL DECLINE';
                statusText = 'Request remains open for other checkers';
            } else {
                logTitle = approvalType.toUpperCase();
                statusText = requestData.status || 'Updated';
            }

            const actionLogEmbed = new EmbedBuilder()
                .setColor(logColor)
                .setTitle(`📝 Checking Request - ${logTitle}`)
                .setDescription(`Checker has responded to checking request.`)
                .addFields(
                    { name: '👤 User', value: `${requester.username} (\`${requester.id}\`)`, inline: true },
                    { name: '👮 Checker', value: `${approver.username}`, inline: true },
                    { name: '🔄 Action', value: logTitle, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '⏰ Time', value: delayMinutes > 0 ? `${delayMinutes} minutes delay` : 'Immediate', inline: true },
                    { name: '📊 Status', value: statusText, inline: true }
                )
                .setFooter({ text: `Request ID: ${requestId} • Checking Logs` })
                .setTimestamp();

            await logsChannel.send({ embeds: [actionLogEmbed] });
        }

        return true;

    } catch (error) {
        console.error('Error handling checking approval:', error);
        return false;
    }
}

// Function to convert pending checking requests to tickets after 10 days
async function convertToTicket(requestId, guild) {
    try {
        const requestData = checkingRequests.get(requestId);
        ...
        // Update status to ticket
        requestData.status = 'ticket';
        requestData.convertedAt = Date.now();
        checkingRequests.set(requestId, requestData);

        // Send DM about conversion to ticket
        const ticketDM = new EmbedBuilder()
            .setColor('#9B59B6')
            .setTitle('🎫 Checking Request Converted to Ticket')
            .setDescription(`Your checking request has been converted to a ticket after 10 days.`)
            .addFields(
                { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                { name: '🎫 Ticket ID', value: `\`${requestId}\``, inline: true },
                { name: '📊 Status', value: 'Can still be acknowledged', inline: true },
                { name: '💡 Note', value: 'Checkers can still approve this request anytime', inline: false }
            )
            .setFooter({ text: 'Checking Service - Ticket System' })
            .setTimestamp();

        try {
            await requester.send({ embeds: [ticketDM] });
        } catch (dmError) {
            console.log('Could not send ticket DM:', dmError.message);
        }

        // Log the conversion
        if (logsChannel) {
            const conversionLogEmbed = new EmbedBuilder()
                .setColor('#9B59B6')
                .setTitle('🎫 Request Converted to Ticket')
                .setDescription(`Checking request automatically converted to ticket after 10 days.`)
                .addFields(
                    { name: '👤 User', value: `${requester.username} (\`${requester.id}\`)`, inline: true },
                    { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                    { name: '🎫 Ticket ID', value: `\`${requestId}\``, inline: true },
                    { name: '⏰ Original Request', value: `<t:${Math.floor(requestData.timestamp / 1000)}:F>`, inline: true },
                    { name: '⏰ Converted At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                    { name: '📊 Status', value: 'Available for acknowledgement', inline: true }
                )
                .setFooter({ text: `Ticket ID: ${requestId} • Auto-Conversion` })
                .setTimestamp();

            await logsChannel.send({ embeds: [conversionLogEmbed] });
        }

        // Schedule final cleanup after another 10 days if still not acknowledged
        setTimeout(async () => {
            const currentData = checkingRequests.get(requestId);
            if (currentData && currentData.status === 'ticket') {
                checkingRequests.delete(requestId);
                console.log(`🧹 Cleaned up old ticket: ${requestId}`);
            }
        }, 10 * 24 * 60 * 60 * 1000); // 10 more days

        return true;

    } catch (error) {
        console.error('Error converting request to ticket:', error);
        return false;
    }
}
...
// ULTIMATE ANTI-NUKE EVENT LISTENERS - ZERO TOLERANCE
client.on('channelCreate', async (channel) => {
    try {
        const guild = channel.guild;
        const auditLogs = await guild.fetchAuditLogs({ limit: 1, type: 10 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            console.log(`🚨 UNAUTHORIZED CHANNEL CREATE: ${channel.name} by ${auditEntry.executor.username}`);
            
            // Delete the channel immediately
            try {
                await channel.delete('ANTI-NUKE: Unauthorized channel creation');
                console.log(`✅ Deleted unauthorized channel: ${channel.name}`);
            } catch (deleteError) {
                console.error('Error deleting unauthorized channel:', deleteError);
            }
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED CHANNEL CREATE', `Attempted to create channel: ${channel.name}`);
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'CHANNEL CREATE BLOCKED', `Unauthorized channel "${channel.name}" created and deleted. User banned.`, auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in channelCreate handler:', error);
    }
});
...
client.on('channelDelete', async (channel) => {
    try {
        ...
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            console.log(`🚨 UNAUTHORIZED CHANNEL DELETE: ${channel.name} by ${auditEntry.executor.username}`);
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED CHANNEL DELETE', `Deleted channel: ${channel.name}`);
            
            // Auto-restore from template
            await autoRestoreServer(guild, `Channel delete violation by ${auditEntry.executor.username}`, auditEntry.executor);
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'CHANNEL DELETE BLOCKED', `Channel "${channel.name}" deleted. User banned and server restored.`, auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in channelDelete handler:', error);
    }
});
...
client.on('channelUpdate', async (oldChannel, newChannel) => {
    try {
        ...
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            const changes = [];
            if (oldChannel.name !== newChannel.name) changes.push('name');
            if (oldChannel.position !== newChannel.position) changes.push('position');
            if (oldChannel.topic !== newChannel.topic) changes.push('topic');
            
            if (changes.length > 0) {
                console.log(`🚨 UNAUTHORIZED CHANNEL UPDATE: ${newChannel.name} by ${auditEntry.executor.username} - ${changes.join(', ')}`);
                
                // Ban the violator
                await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED CHANNEL UPDATE', `Modified channel: ${newChannel.name} (${changes.join(', ')})`);
                
                // Auto-restore from template
                await autoRestoreServer(guild, `Channel update violation by ${auditEntry.executor.username}`, auditEntry.executor);
                
                // Send critical alert
                await sendCriticalSecurityAlert(guild, 'CHANNEL UPDATE BLOCKED', `Channel "${newChannel.name}" modified. User banned and changes reverted.`, auditEntry.executor);
            }
        }
    } catch (error) {
...
client.on('roleCreate', async (role) => {
    try {
        ...
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            // Check if executor is an authorized bot
            if (auditEntry.executor.bot && WHITELISTED_BOTS.has(auditEntry.executor.id)) {
                console.log(`✅ AUTHORIZED BOT ROLE CREATE: ${role.name} by authorized bot ${auditEntry.executor.username} - ALLOWING`);
                return; // Allow authorized bots to create roles
            }
            
            console.log(`🚨 UNAUTHORIZED ROLE CREATE: ${role.name} by ${auditEntry.executor.username}`);
            
            // Delete the role immediately
            try {
                await role.delete('ANTI-NUKE: Unauthorized role creation');
                console.log(`✅ Deleted unauthorized role: ${role.name}`);
            } catch (deleteError) {
                console.error('Error deleting unauthorized role:', deleteError);
            }
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED ROLE CREATE', `Attempted to create role: ${role.name}`);
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'ROLE CREATE BLOCKED', `Unauthorized role "${role.name}" created and deleted. User banned.`, auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in roleCreate handler:', error);
    }
});
...
client.on('roleDelete', async (role) => {
    try {
        const guild = role.guild;
        const auditLogs = await guild.fetchAuditLogs({ limit: 1, type: 32 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            console.log(`🚨 UNAUTHORIZED ROLE DELETE: ${role.name} by ${auditEntry.executor.username}`);
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED ROLE DELETE', `Deleted role: ${role.name}`);
            
            // Auto-restore from template
            await autoRestoreServer(guild, `Role delete violation by ${auditEntry.executor.username}`, auditEntry.executor);
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'ROLE DELETE BLOCKED', `Role "${role.name}" deleted. User banned and server restored.`, auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in roleDelete handler:', error);
    }
});
...
client.on('roleUpdate', async (oldRole, newRole) => {
    try {
        ...
        const auditLogs = await guild.fetchAuditLogs({ limit: 1, type: 31 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            // Check if executor is an authorized bot
            if (auditEntry.executor.bot && WHITELISTED_BOTS.has(auditEntry.executor.id)) {
                console.log(`✅ AUTHORIZED BOT ROLE UPDATE: ${newRole.name} by authorized bot ${auditEntry.executor.username} - ALLOWING`);
                return; // Allow authorized bots to update roles
            }
            ...
            if (changes.length > 0) {
                console.log(`🚨 UNAUTHORIZED ROLE UPDATE: ${newRole.name} by ${auditEntry.executor.username} - ${changes.join(', ')}`);
                
                // Ban the violator
                await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED ROLE UPDATE', `Modified role: ${newRole.name} (${changes.join(', ')})`);
                
                // Auto-restore from template
                await autoRestoreServer(guild, `Role update violation by ${auditEntry.executor.username}`, auditEntry.executor);
                
                // Send critical alert
                await sendCriticalSecurityAlert(guild, 'ROLE UPDATE BLOCKED', `Role "${newRole.name}" modified. User banned and changes reverted.`, auditEntry.executor);
            }
        }
    } catch (error) {
        console.error('Error in roleUpdate handler:', error);
    }
});
...
client.on('webhookUpdate', async (channel) => {
    try {
        const guild = channel.guild;
        const auditLogs = await guild.fetchAuditLogs({ limit: 1, type: 50 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            console.log(`🚨 UNAUTHORIZED WEBHOOK CREATE/UPDATE: ${channel.name} by ${auditEntry.executor.username}`);
            
            // Delete all webhooks in the channel
            try {
                const webhooks = await channel.fetchWebhooks();
                for (const webhook of webhooks.values()) {
                    await webhook.delete('ANTI-NUKE: Unauthorized webhook');
                }
                console.log(`✅ Deleted all webhooks in channel: ${channel.name}`);
            } catch (deleteError) {
                console.error('Error deleting webhooks:', deleteError);
            }
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED WEBHOOK CREATE', `Created webhook in channel: ${channel.name}`);
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'WEBHOOK BLOCKED', `Unauthorized webhook in "${channel.name}" deleted. User banned.`, auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in webhookUpdate handler:', error);
    }
});

client.on('guildUpdate', async (oldGuild, newGuild) => {
    try {
        const auditLogs = await newGuild.fetchAuditLogs({ limit: 1, type: 1 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, newGuild.id)) {
            const changes = [];
            if (oldGuild.name !== newGuild.name) changes.push('name');
            if (oldGuild.iconURL() !== newGuild.iconURL()) changes.push('icon');
            if (oldGuild.bannerURL() !== newGuild.bannerURL()) changes.push('banner');
            
            if (changes.length > 0) {
                console.log(`🚨 UNAUTHORIZED SERVER UPDATE by ${auditEntry.executor.username} - ${changes.join(', ')}`);
                
                // Ban the violator
                await handleProtectionViolation(newGuild, auditEntry.executor, 'UNAUTHORIZED SERVER UPDATE', `Modified server: ${changes.join(', ')}`);
                
                // Auto-restore from template
                await autoRestoreServer(newGuild, `Server update violation by ${auditEntry.executor.username}`, auditEntry.executor);
                
                // Send critical alert
                await sendCriticalSecurityAlert(newGuild, 'SERVER UPDATE BLOCKED', `Server settings modified. User banned and settings restored.`, auditEntry.executor);
            }
        }
    } catch (error) {
        console.error('Error in guildUpdate handler:', error);
    }
});

client.on('guildIntegrationsUpdate', async (guild) => {
    try {
        const auditLogs = await guild.fetchAuditLogs({ limit: 1, type: 80 });
        const auditEntry = auditLogs.entries.first();
        
        if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
            console.log(`🚨 UNAUTHORIZED INTEGRATION UPDATE by ${auditEntry.executor.username}`);
            
            // Try to remove unauthorized integrations
            try {
                const integrations = await guild.fetchIntegrations();
                for (const integration of integrations.values()) {
                    if (integration.application && !WHITELISTED_BOTS.has(integration.application.id)) {
                        await integration.delete('ANTI-NUKE: Unauthorized integration');
                        console.log(`✅ Removed unauthorized integration: ${integration.name}`);
                    }
                }
            } catch (integrationError) {
                console.error('Error removing integrations:', integrationError);
            }
            
            // Ban the violator
            await handleProtectionViolation(guild, auditEntry.executor, 'UNAUTHORIZED INTEGRATION UPDATE', 'Attempted to modify server integrations');
            
            // Send critical alert
            await sendCriticalSecurityAlert(guild, 'INTEGRATION BLOCKED', 'Unauthorized integration update blocked. User banned.', auditEntry.executor);
        }
    } catch (error) {
        console.error('Error in guildIntegrationsUpdate handler:', error);
    }
});
...
client.once('ready', async () => {
    ...
    client.guilds.cache.forEach(async guild => {
        ...
        const startupEmbed = new EmbedBuilder()
            .setColor('#000000')
            .setTitle('⚫ ULTIMATE ANTI-NUKE SYSTEM ACTIVATED')
            .setDescription(`**ZERO TOLERANCE SECURITY** - Your Discord server is now protected with the most powerful anti-nuke system ever created!`)
            .addFields(
                { name: '📱 WhatsApp Alerts', value: `Active: ${WHATSAPP_ALERT_NUMBER}`, inline: true },
                { name: '⚡ Response Time', value: '< 0.1 millisecond', inline: true },
                { name: '🔄 Server Scanning', value: 'Real-time monitoring', inline: true },
                { name: '🛡️ Ultimate Protection Features', value: '• **ZERO** unauthorized channel create/delete\n• **ZERO** unauthorized role modifications\n• **ZERO** webhook creation allowed\n• **ZERO** integration bypasses\n• **ZERO** permission updates\n• **INSTANT** permanent bans', inline: false },
                { name: '🚨 Security Level', value: '**ULTIMATE ANTI-NUKE** - Impossible to bypass', inline: true },
                { name: '📊 Log Channel', value: `<#${LOGS_CHANNEL_ID}>`, inline: true },
                { name: '🔍 Real-time Monitoring', value: '• Channel create/delete detection\n• Role create/delete/update detection\n• Webhook creation blocking\n• Integration update blocking\n• Permission change monitoring\n• Server settings protection', inline: false },
                { name: '👑 Authorized Users', value: `**Bot Owner:** <@${BOT_OWNER_ID}>\n**Server Owner:** <@${guild.ownerId}>\n**NOBODY ELSE CAN MAKE ANY CHANGES!**`, inline: false },
                { name: '⚠️ ZERO TOLERANCE POLICY', value: '**ANY UNAUTHORIZED ACTION = INSTANT PERMANENT BAN**\n• NO warnings given\n• NO quarantine period\n• IMMEDIATE permanent ban\n• AUTO server restoration\n• WhatsApp instant alerts', inline: false }
            )
            .setFooter({ text: 'Ultimate Anti-Nuke by Script.AGI - Zero Tolerance Guarantee' })
            .setTimestamp();
    ...
    // Send/update checking request widget on startup
    console.log('Setting up checking request widgets...');
    for (const guild of client.guilds.cache.values()) {
        try {
            const result = await sendCheckingRequestWidget(guild);
            if (result) {
                console.log(`✅ Checking request widget successfully set up in guild: ${guild.name}`);
            } else {
                console.log(`❌ Failed to set up checking request widget in guild: ${guild.name}`);
            }
        } catch (error) {
            console.error(`Error setting up checking request widget in guild ${guild.name}:`, error);
        }
    }
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    try {
        ...
        // Also skip if this is a role change made by an authorized bot
        if (oldMember.roles.cache.size !== newMember.roles.cache.size) {
            try {
                const auditLogs = await newMember.guild.fetchAuditLogs({ limit: 1, type: 25 }); // MEMBER_ROLE_UPDATE
                const auditEntry = auditLogs.entries.first();
                
                if (auditEntry && auditEntry.executor.bot && WHITELISTED_BOTS.has(auditEntry.executor.id)) {
                    console.log(`✅ AUTHORIZED BOT MEMBER UPDATE: ${auditEntry.executor.username} modified roles for ${newMember.user.username} - ALLOWING`);
                    return; // Allow authorized bots to modify member roles
                }
            } catch (auditError) {
                console.log('Could not check audit logs for member update:', auditError.message);
            }
        }
...
// Enhanced audit log monitoring with comprehensive protection
client.on('guildAuditLogEntryCreate', async (auditLogEntry, guild) => {
    setImmediate(async () => {
        try {
            const { executor, action, target, changes } = auditLogEntry;

            // Skip if executor is immune (ONLY OWNER AND BOT)
            if (!executor || isProtectionImmune(executor.id, guild.id)) return;
            ...
            console.log(`🔍 AUDIT LOG: Action ${action} by ${executor.username} (${executor.id}) - Bot: ${executor.bot}`);

            // COMPREHENSIVE ACTION MONITORING - ZERO TOLERANCE
            switch (action) {
                // CHANNEL ACTIONS - IMMEDIATE BAN
                case 10: // CHANNEL_CREATE
                    violationType = 'UNAUTHORIZED_CHANNEL_CREATE';
                    details = `Created channel: ${target?.name || 'Unknown'}`;
                    shouldAutoRestore = true;
                    console.log(`🚨 CHANNEL CREATE VIOLATION: ${executor.username} created ${target?.name}`);
                    break;
                    
                case 12: // CHANNEL_DELETE  
                    violationType = 'UNAUTHORIZED_CHANNEL_DELETE';
                    details = `Deleted channel: ${target?.name || 'Unknown'}`;
                    shouldAutoRestore = true;
                    console.log(`🚨 CHANNEL DELETE VIOLATION: ${executor.username} deleted ${target?.name}`);
                    break;
                    
                case 11: // CHANNEL_UPDATE
                    const channelChanges = changes?.map(c => c.key).join(', ') || 'Unknown';
                    if (channelChanges.includes('name') || channelChanges.includes('position') || channelChanges.includes('topic')) {
                        violationType = 'UNAUTHORIZED_CHANNEL_UPDATE';
                        details = `Modified channel: ${target?.name} (${channelChanges})`;
                        shouldAutoRestore = true;
                        console.log(`🚨 CHANNEL UPDATE VIOLATION: ${executor.username} modified ${target?.name} - ${channelChanges}`);
                    }
                    break;
                ...
                case 15: // CHANNEL_OVERWRITE_DELETE
                    violationType = 'UNAUTHORIZED_PERMISSION_CHANGE';
                    details = `Modified channel permissions: ${target?.name || 'Unknown'}`;
                    shouldAutoRestore = true;
                    console.log(`🚨 PERMISSION VIOLATION: ${executor.username} modified permissions for ${target?.name}`);
                    break;

                // ROLE ACTIONS - IMMEDIATE BAN (BUT ALLOW AUTHORIZED BOTS)
                case 30: // ROLE_CREATE
                    // Check if executor is an authorized bot
                    if (executor.bot && WHITELISTED_BOTS.has(executor.id)) {
                        console.log(`✅ AUTHORIZED BOT ROLE CREATE: ${executor.username} created role ${target?.name} - ALLOWING`);
                        shouldBan = false; // Don't ban authorized bots
                        break;
                    }
                    violationType = 'UNAUTHORIZED_ROLE_CREATE';
                    details = `Created role: ${target?.name || 'Unknown'}`;
                    shouldAutoRestore = true;
                    console.log(`🚨 ROLE CREATE VIOLATION: ${executor.username} created role ${target?.name}`);
                    break;
                    
                case 32: // ROLE_DELETE
                    // Check if executor is an authorized bot
                    if (executor.bot && WHITELISTED_BOTS.has(executor.id)) {
                        console.log(`✅ AUTHORIZED BOT ROLE DELETE: ${executor.username} deleted role ${target?.name} - ALLOWING`);
                        shouldBan = false; // Don't ban authorized bots
                        break;
                    }
                    violationType = 'UNAUTHORIZED_ROLE_DELETE';
                    details = `Deleted role: ${target?.name || 'Unknown'}`;
                    shouldAutoRestore = true;
                    console.log(`🚨 ROLE DELETE VIOLATION: ${executor.username} deleted role ${target?.name}`);
                    break;
                    
                case 31: // ROLE_UPDATE
                    // Check if executor is an authorized bot
                    if (executor.bot && WHITELISTED_BOTS.has(executor.id)) {
                        console.log(`✅ AUTHORIZED BOT ROLE UPDATE: ${executor.username} modified role ${target?.name} - ALLOWING`);
                        shouldBan = false; // Don't ban authorized bots
                        break;
                    }
                    const roleChanges = changes?.map(c => c.key).join(', ') || 'Unknown';
                    violationType = 'UNAUTHORIZED_ROLE_UPDATE';
                    details = `Modified role: ${target?.name} (${roleChanges})`;
                    shouldAutoRestore = true;
                    console.log(`🚨 ROLE UPDATE VIOLATION: ${executor.username} modified role ${target?.name} - ${roleChanges}`);
                    break;

                // BOT INTEGRATION - IMMEDIATE REMOVAL AND BAN INVITER (ONLY IF UNAUTHORIZED USER)
                case 28: // BOT_ADD
                    if (target?.bot && !WHITELISTED_BOTS.has(target.id)) {
                        // Check if the person adding the bot is authorized
                        if (!isProtectionImmune(executor.id, guild.id)) {
                            violationType = 'UNAUTHORIZED_BOT_ADD';
                            details = `Added unauthorized bot: ${target.username} (${target.id}) - Both bot and inviter will be banned`;
                            shouldBan = true; // Ban the person who added the bot
                            console.log(`🚨 UNAUTHORIZED BOT ADD BY UNAUTHORIZED USER: ${executor.username} added bot ${target.username} - BANNING BOTH`);
                            
                            // Immediate bot removal and inviter punishment
                            setImmediate(async () => {
                                try {
                                    const botMember = guild.members.cache.get(target.id);
                                    if (botMember) {
                                        await handleUnauthorizedBot(guild, botMember, executor);
                                        console.log(`⚡ REMOVED UNAUTHORIZED BOT AND BANNED INVITER: Bot: ${target.username}, Inviter: ${executor.username}`);
                                    }
                                } catch (error) {
                                    console.error('Error removing unauthorized bot:', error);
                                }
                            });
                        } else {
                            // Authorized user added non-whitelisted bot - allow but log
                            console.log(`✅ AUTHORIZED USER ADDED NON-WHITELISTED BOT: ${executor.username} added ${target.username} - ALLOWING`);
                            shouldBan = false; // Don't ban authorized users
                        }
                    }
                    break;

                // INTEGRATION ACTIONS - IMMEDIATE BAN
                case 80: // INTEGRATION_CREATE
                case 81: // INTEGRATION_UPDATE
                case 82: // INTEGRATION_DELETE
                    violationType = 'UNAUTHORIZED_INTEGRATION_ACTION';
                    details = `Integration action detected`;
                    console.log(`🚨 INTEGRATION VIOLATION: ${executor.username} performed integration action`);
                    break;

                // WEBHOOK ACTIONS - IMMEDIATE BAN
                case 50: // WEBHOOK_CREATE
                case 51: // WEBHOOK_UPDATE
                case 52: // WEBHOOK_DELETE
                    violationType = 'UNAUTHORIZED_WEBHOOK_ACTION';
                    details = `Webhook action detected`;
                    console.log(`🚨 WEBHOOK VIOLATION: ${executor.username} performed webhook action`);
                    break;

                // SERVER SETTINGS - IMMEDIATE BAN
                case 1: // GUILD_UPDATE
                    const serverChanges = changes?.map(c => c.key).join(', ') || 'Unknown';
                    violationType = 'UNAUTHORIZED_SERVER_UPDATE';
                    details = `Modified server settings: ${serverChanges}`;
                    console.log(`🚨 SERVER UPDATE VIOLATION: ${executor.username} modified server - ${serverChanges}`);
                    break;
            }

            // EXECUTE IMMEDIATE PUNISHMENT
            if (violationType && shouldBan) {
                console.log(`🚨 EXECUTING IMMEDIATE BAN: ${violationType} by ${executor.username}`);
                
                setImmediate(async () => {
                    try {
                        // IMMEDIATE PERMANENT BAN
                        await guild.bans.create(executor.id, {
                            reason: `ANTI-NUKE: ${violationType} - ${details}`,
                            deleteMessageSeconds: 7 * 24 * 60 * 60
                        });
                        
                        console.log(`✅ BANNED: ${executor.username} for ${violationType}`);
                        
                        // Auto-restore if needed
                        if (shouldAutoRestore) {
                            await autoRestoreServer(guild, violationType, executor);
                            console.log(`✅ AUTO-RESTORED: Server after ${violationType}`);
                        }
                        ...
                        await sendCriticalSecurityAlert(
                            guild,
                            violationType,
                            `${executor.username} (${executor.id}) - ${details}`,
                            executor
                        );
                        ...
                        const violationEmbed = new EmbedBuilder()
                            .setColor('#FF0000')
                            .setTitle('🚨 ANTI-NUKE VIOLATION - IMMEDIATE BAN')
                            .setDescription(`**ZERO TOLERANCE SECURITY BREACH**`)
                            .addFields(
                                { name: '🎯 Violator', value: `${executor.username}\n\`${executor.id}\``, inline: true },
                                { name: '⚠️ Violation', value: violationType, inline: true },
                                { name: '⏰ Action', value: 'PERMANENT BAN', inline: true },
                                { name: '📝 Details', value: details, inline: false },
                                { name: '🛡️ Status', value: shouldAutoRestore ? 'Server restored' : 'Threat neutralized', inline: false }
                            )
                            .setFooter({ text: 'Anti-Nuke System - Zero Tolerance' })
                            .setTimestamp();

                        await sendLogMessage(guild, violationEmbed);
                        
                    } catch (error) {
                        console.error(`Error executing punishment for ${violationType}:`, error);
                    }
                });
            }

        } catch (error) {
            console.error('Error in enhanced audit log monitoring:', error);
        }
    });
});

// Enhanced member join monitoring for unauthorized bot detection
client.on('guildMemberAdd', async member => {
    ...
    if (!WHITELISTED_BOTS.has(member.user.id)) {
        console.log(`🚨 NON-WHITELISTED BOT JOINED: ${member.user.username} (${member.user.id}) - Checking inviter...`);
        
        setImmediate(async () => {
            try {
                // Get who added the bot from audit logs
                const auditLogs = await guild.fetchAuditLogs({
                    type: 28, // BOT_ADD
                    limit: 5
                });
                ...
                // Check if inviter is authorized - only remove bot if inviter is not authorized
                if (inviter && isProtectionImmune(inviter.id, guild.id)) {
                    console.log(`✅ AUTHORIZED USER ADDED BOT: ${inviter.username} added ${member.user.username} - ALLOWING`);
                    return; // Allow the bot to stay
                }
                
                // Only remove bot if added by unauthorized user
                console.log(`🚨 UNAUTHORIZED USER ADDED BOT: ${inviter?.username || 'Unknown'} - Removing bot and banning both`);
                await handleUnauthorizedBot(guild, member, inviter);
                
                console.log(`⚡ REMOVED UNAUTHORIZED BOT AND BANNED INVITER: ${member.user.username}`);
                
            } catch (error) {
                console.error('Error handling unauthorized bot join:', error);
            }
        });
    } else {
        console.log(`✅ WHITELISTED BOT JOINED: ${member.user.username} - Allowed`);
    }
});

// Real-time event monitoring for immediate protection
client.on('channelCreate', async channel => {
    setImmediate(async () => {
        try {
            const guild = channel.guild;
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Channel created by ${auditEntry.executor.username} - IMMEDIATE DELETION`);
                
                // Delete the channel immediately
                await channel.delete('ANTI-NUKE: Unauthorized channel creation').catch(console.error);
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Created unauthorized channel ${channel.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                console.log(`✅ BANNED AND DELETED: ${auditEntry.executor.username} for channel creation`);
            }
        } catch (error) {
            console.error('Error in real-time channel create protection:', error);
        }
    });
});
...
client.on('channelDelete', async channel => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Channel deleted by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Deleted channel ${channel.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                // Auto-restore server
                await autoRestoreServer(guild, 'Channel deletion', auditEntry.executor);
                
                console.log(`✅ BANNED AND RESTORED: ${auditEntry.executor.username} for channel deletion`);
            }
        } catch (error) {
            console.error('Error in real-time channel delete protection:', error);
        }
    });
});
    ...
    setImmediate(async () => {
        try {
            ...
            if (oldChannel.name !== newChannel.name) {
                ...
                if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                    console.log(`🚨 REAL-TIME: Channel name changed by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                    
                    // Restore channel name
                    await newChannel.setName(oldChannel.name, 'ANTI-NUKE: Reverting unauthorized name change').catch(console.error);
                    
                    // Ban the violator
                    await guild.bans.create(auditEntry.executor.id, {
                        reason: `ANTI-NUKE: Changed channel name from ${oldChannel.name} to ${newChannel.name}`,
                        deleteMessageSeconds: 7 * 24 * 60 * 60
                    }).catch(console.error);
                    
                    console.log(`✅ BANNED AND REVERTED: ${auditEntry.executor.username} for channel name change`);
                }
            }
        } catch (error) {
            console.error('Error in real-time channel update protection:', error);
        }
    });
});
...
client.on('roleCreate', async role => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Role created by ${auditEntry.executor.username} - IMMEDIATE DELETION`);
                
                // Delete the role immediately
                await role.delete('ANTI-NUKE: Unauthorized role creation').catch(console.error);
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Created unauthorized role ${role.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                console.log(`✅ BANNED AND DELETED: ${auditEntry.executor.username} for role creation`);
            }
        } catch (error) {
            console.error('Error in real-time role create protection:', error);
        }
    });
});
...
client.on('roleDelete', async role => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Role deleted by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Deleted role ${role.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                // Auto-restore server
                await autoRestoreServer(guild, 'Role deletion', auditEntry.executor);
                
                console.log(`✅ BANNED AND RESTORED: ${auditEntry.executor.username} for role deletion`);
            }
        } catch (error) {
            console.error('Error in real-time role delete protection:', error);
        }
    });
});
...
client.on('roleUpdate', async (oldRole, newRole) => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Role updated by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Modified role ${newRole.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                // Auto-restore server
                await autoRestoreServer(guild, 'Role modification', auditEntry.executor);
                
                console.log(`✅ BANNED AND RESTORED: ${auditEntry.executor.username} for role modification`);
            }
        } catch (error) {
            console.error('Error in real-time role update protection:', error);
        }
    });
});
...
client.on('webhookUpdate', async channel => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Webhook created by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Delete all webhooks in the channel
                const webhooks = await channel.fetchWebhooks();
                for (const webhook of webhooks.values()) {
                    await webhook.delete('ANTI-NUKE: Unauthorized webhook').catch(console.error);
                }
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Created unauthorized webhook in ${channel.name}`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                console.log(`✅ BANNED AND REMOVED WEBHOOKS: ${auditEntry.executor.username}`);
            }
        } catch (error) {
            console.error('Error in real-time webhook protection:', error);
        }
    });
});
...
client.on('guildUpdate', async (oldGuild, newGuild) => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, newGuild.id)) {
                console.log(`🚨 REAL-TIME: Server updated by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Ban the violator
                await newGuild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Modified server settings`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                // Auto-restore server
                await autoRestoreServer(newGuild, 'Server settings change', auditEntry.executor);
                
                console.log(`✅ BANNED AND RESTORED: ${auditEntry.executor.username} for server modification`);
            }
        } catch (error) {
            console.error('Error in real-time guild update protection:', error);
        }
    });
});
...
client.on('guildIntegrationsUpdate', async guild => {
    setImmediate(async () => {
        try {
            ...
            if (auditEntry && !isProtectionImmune(auditEntry.executor.id, guild.id)) {
                console.log(`🚨 REAL-TIME: Integration updated by ${auditEntry.executor.username} - IMMEDIATE BAN`);
                
                // Remove unauthorized integrations
                const integrations = await guild.fetchIntegrations();
                for (const integration of integrations.values()) {
                    if (integration.application && !WHITELISTED_BOTS.has(integration.application.id)) {
                        await integration.delete('ANTI-NUKE: Unauthorized integration').catch(console.error);
                    }
                }
                
                // Ban the violator
                await guild.bans.create(auditEntry.executor.id, {
                    reason: `ANTI-NUKE: Modified server integrations`,
                    deleteMessageSeconds: 7 * 24 * 60 * 60
                }).catch(console.error);
                
                console.log(`✅ BANNED AND REMOVED INTEGRATIONS: ${auditEntry.executor.username}`);
            }
        } catch (error) {
            console.error('Error in real-time integration protection:', error);
        }
    });
});
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        // Handle checking request
        if (interaction.customId === 'request_checking') {
            // Create device selection buttons
            const mobileButton = new ButtonBuilder()
                .setCustomId('checking_device_mobile')
                .setLabel('📱 Mobile Checking')
                .setStyle(ButtonStyle.Primary);

            const pcButton = new ButtonBuilder()
                .setCustomId('checking_device_pc')
                .setLabel('💻 PC Checking')
                .setStyle(ButtonStyle.Primary);

            const deviceRow = new ActionRowBuilder()
                .addComponents(mobileButton, pcButton);

            const deviceEmbed = new EmbedBuilder()
                .setColor('#00FF00')
                .setTitle('📱 Select Your Device')
                .setDescription('Please select the device you want to be checked on:')
                .addFields(
                    { name: '📱 Mobile', value: 'For mobile device checking', inline: true },
                    { name: '💻 PC', value: 'For PC/computer checking', inline: true }
                )
                .setFooter({ text: 'Checking Request System' })
                .setTimestamp();
        ...
        // Handle device selection for checking
        if (interaction.customId.startsWith('checking_device_')) {
            const device = interaction.customId.replace('checking_device_', '');
            const requestId = `${interaction.user.id}_${Date.now()}`;

            // Store the request
            checkingRequests.set(requestId, {
            ...
            const approvalMessage = await createCheckingApprovalWidget(requestId, interaction.guild, interaction.user, device);

            // Notify checkers
            await notifyCheckers(interaction.guild, interaction.user, device, requestId);

            // Send confirmation to user
            const confirmationEmbed = new EmbedBuilder()
                .setColor('#00FF00')
                .setTitle('✅ Checking Request Submitted')
                .setDescription('Your checking request has been submitted successfully!')
                .addFields(
                    { name: '📱 Device', value: device.toUpperCase(), inline: true },
                    { name: '🆔 Request ID', value: `\`${requestId}\``, inline: true },
                    { name: '📊 Status', value: 'Pending checker approval', inline: true },
                    { name: '🔔 Notifications', value: 'Checkers have been notified via DM', inline: true },
                    { name: '📍 Approval', value: `<#${CHECKING_APPROVAL_CHANNEL_ID}>`, inline: true },
                    { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: true },
                    { name: '⏰ Next Steps', value: 'Wait for checker approval. You will receive a DM when approved.', inline: false }
                )
                .setFooter({ text: 'Checking Request System - Request Submitted' })
                .setTimestamp();
            ...
            // Log the request
            const logsChannel = interaction.guild.channels.cache.get(CHECKING_LOGS_CHANNEL_ID);
            if (logsChannel) {
                const logEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('📝 New Checking Request')
                    .setDescription('A user has submitted a new checking request.')
                    .addFields(
                        ...
                        { name: '🔔 Checkers Notified', value: 'Yes', inline: true }
                    )
                    .setFooter({ text: `Request ID: ${requestId} • New Request` })
                    .setTimestamp();

                await logsChannel.send({ embeds: [logEmbed] });
            }

            // Schedule automatic conversion to ticket after 10 days
            setTimeout(async () => {
                const requestData = checkingRequests.get(requestId);
                if (requestData && requestData.status === 'pending') {
                    await convertToTicket(requestId, interaction.guild);
                }
        ...
        // Handle checking approval buttons
        if (interaction.customId.startsWith('approve_checking_')) {
            const requestId = interaction.customId.replace('approve_checking_', '');
            
            // Check if user has checker role
            const member = interaction.guild.members.cache.get(interaction.user.id);
            if (!member.roles.cache.has(CHECKER_ROLE_ID)) {
                await interaction.reply({ content: '❌ You need the Checker role to approve requests.', ephemeral: true });
                return;
            }

            await interaction.deferReply({ ephemeral: true });

            const success = await handleCheckingApproval(requestId, interaction.guild, interaction.user, 'approve');
            ...
            if (success) {
                ...
                const updatedEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('✅ Checking Request Approved')
                    .setDescription('This checking request has been approved!')
                    .addFields(
                        { name: '✅ Approved By', value: `${interaction.user.username}`, inline: true },
                        { name: '⏰ Approved At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                        { name: '📊 Status', value: 'User notified and can join voice', inline: true }
                    )
                    .setFooter({ text: `Request ID: ${requestId} • Approved` })
                    .setTimestamp();

                await interaction.message.edit({ embeds: [updatedEmbed], components: [] });
                await interaction.editReply('✅ Checking request approved! User has been notified.');
            } else {
                await interaction.editReply('❌ Error approving checking request.');
            }
            return;
        }

        // Handle checking disapproval
        if (interaction.customId.startsWith('disapprove_checking_')) {
            const requestId = interaction.customId.replace('disapprove_checking_', '');
            
            const member = interaction.guild.members.cache.get(interaction.user.id);
            if (!member.roles.cache.has(CHECKER_ROLE_ID)) {
                await interaction.reply({ content: '❌ You need the Checker role to disapprove requests.', ephemeral: true });
                return;
            }
            ...
            const requestData = checkingRequests.get(requestId);
            if (!requestData) {
                await interaction.editReply('❌ Checking request not found.');
                return;
            }

            // Add this checker to the list of checkers who disapproved
            if (!requestData.disapprovedBy) {
                requestData.disapprovedBy = [];
            }
            
            if (!requestData.disapprovedBy.includes(interaction.user.id)) {
                requestData.disapprovedBy.push(interaction.user.id);
                checkingRequests.set(requestId, requestData);
            }

            // Send disapproval notification but keep request open
            const success = await handleCheckingApproval(requestId, interaction.guild, interaction.user, 'disapprove_individual');

            if (success) {
                // Update the approval message to show this checker's response but keep it open
                const currentEmbed = interaction.message.embeds[0];
                const requester = await client.users.fetch(requestData.userId);
                
                const updatedEmbed = new EmbedBuilder()
                    .setColor('#FFA500') // Keep orange to show it's still pending
                    .setTitle('⏳ Checking Request - Still Awaiting Approval')
                    .setDescription(`**A user has requested checking service**\n\nOne checker has declined, but the request remains open for other checkers.`)
                    .addFields(
                        { name: '👤 User', value: `${requester.username}\n\`${requester.id}\``, inline: true },
                        { name: '📱 Device', value: requestData.device.toUpperCase(), inline: true },
                        { name: '⏰ Requested At', value: `<t:${Math.floor(requestData.timestamp / 1000)}:F>`, inline: true },
                        { name: '🎯 Voice Channel', value: `<#${CHECKING_VOICE_CHANNEL_ID}>`, inline: true },
                        { name: '🆔 Request ID', value: `\`${requestId}\``, inline: true },
                        { name: '📊 Status', value: 'Still pending - Other checkers can approve', inline: true },
                        { name: '❌ Declined By', value: requestData.disapprovedBy.map(id => `<@${id}>`).join(', '), inline: false },
                        { name: '🔍 Action Required', value: 'Other checkers can still approve or set delay time', inline: false }
                    )
                    .setThumbnail(requester.displayAvatarURL({ dynamic: true, size: 128 }))
                    .setFooter({ text: `Checking Request System • ID: ${requestId} • Request remains open` })
                    .setTimestamp();

                // Keep all original buttons active
                await interaction.message.edit({ embeds: [updatedEmbed] });
                await interaction.editReply('✅ You have declined this request, but it remains open for other checkers to approve.');
            } else {
                await interaction.editReply('❌ Error processing disapproval.');
            }
            return;
        }

        // Handle delay buttons
        if (interaction.customId.startsWith('delay_checking_')) {
            const parts = interaction.customId.split('_');
            const requestId = parts.slice(2, -1).join('_'); // Everything except 'delay', 'checking', and the last part (minutes)
            const delayMinutes = parseInt(parts[parts.length - 1]);
            
            const member = interaction.guild.members.cache.get(interaction.user.id);
            if (!member.roles.cache.has(CHECKER_ROLE_ID)) {
                await interaction.reply({ content: '❌ You need the Checker role to set delays.', ephemeral: true });
                return;
            }

            await interaction.deferReply({ ephemeral: true });

            const success = await handleCheckingApproval(requestId, interaction.guild, interaction.user, 'delay', delayMinutes);

            if (success) {
                await interaction.editReply(`✅ Delay of ${delayMinutes} minutes set. User has been notified.`);
            } else {
                await interaction.editReply('❌ Error setting delay.');
            }
            return;
        }

        // Handle busy button
        if (interaction.customId.startsWith('busy_checking_')) {
            const requestId = interaction.customId.replace('busy_checking_', '');
            
            const member = interaction.guild.members.cache.get(interaction.user.id);
            if (!member.roles.cache.has(CHECKER_ROLE_ID)) {
                await interaction.reply({ content: '❌ You need the Checker role to mark as busy.', ephemeral: true });
                return;
            }

            await interaction.deferReply({ ephemeral: true });

            const success = await handleCheckingApproval(requestId, interaction.guild, interaction.user, 'busy');

            if (success) {
                await interaction.editReply('✅ Marked as busy. User has been notified.');
            } else {
                await interaction.editReply('❌ Error marking as busy.');
            }
            return;
        }
...
client.on('guildMemberAdd', async member => {
    ...
    try {
        ...
        if (member.user.bot) {
            ...
            if (!WHITELISTED_BOTS.has(member.user.id)) {
                console.log(`🚨 NUKE BOT DETECTED - MAXIMUM SPEED NEUTRALIZATION: ${member.user.username}`);
